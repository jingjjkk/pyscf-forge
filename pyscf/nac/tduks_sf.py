...
def get_Hellmann_Feymann(td_grad, x_y_I, x_y_J, atmlst=None, max_memory=6000, verbose=logger.INFO,
    ...
    if with_k:
        ...
        vk1_I, vk1_J = _cached_get_k(cache_mgr, mf, mol, (dmt_I, dmt_J), hermi=0) * hyb
        vk0 = vk0 * hyb
        if omega != 0:
            vk0 += _cached_get_k(cache_mgr, mf, mol, (dmzooa, dmzoob), hermi=1, omega=omega) * (alpha - hyb)
            vk1_I_omega, vk1_J_omega = _cached_get_k(cache_mgr, mf, mol, (dmt_I, dmt_J), hermi=0, omega=omega)
    ...
    if with_k:
        ...
        vk1_all = -_cached_get_k(cache_mgr, td_grad, mol, (dmt_I, dmt_I.T, dmt_J, dmt_J.T)) * hyb
        vk1_I = vk1_all[:2]
        vk1_J = vk1_all[2:]
        if omega != 0:
            ...
            vk1_all_omega = -_cached_get_k(cache_mgr, td_grad, mol, (dmt_I, dmt_I.T, dmt_J, dmt_J.T), omega=omega)
