...
```bash
node scripts/review-wait.mjs DoraemonHugU/oh-my-harness#2
```
...
- 有新的 `inline review comments` 时，加载 `receiving-code-review` skill 处理审查反馈。
- 处理相关 `inline review comments` 时，在线程内回复；不要用顶级评论代替。
